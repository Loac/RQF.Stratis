...
if (count _this == 2) then { //init
	...
	_priority = 1000;
	{
		...
		_box addaction [format["<t color='#FF0000'>Открыть снаряжения %1</t>",_file],"HA\weapon\core\bgr_action.sqf",[1,_file,0],_priority,true,false,"",format ['_target getVariable ["bgr_curent_ammo_template",""]!="%1"',_file]];
		_box addaction [format["<t color='#FF0000'>Закрыть снаряжения %1</t>",_file],"HA\weapon\core\bgr_action.sqf",[2,_file,0],_priority-1,true,false,"",format ['_target getVariable ["bgr_curent_ammo_template",""]=="%1"',_file]];
		_actions = _template select 1;
		{
			_box addaction [format["   Снаряжение %1 (%2)",(_x select 1),_file],"HA\weapon\core\bgr_action.sqf",[3,_file,(_x select 0)],_priority-5,true,true,"",format ['_target getVariable ["bgr_curent_ammo_template",""]=="%1"',_file]];
		} foreach _actions;
	} foreach _templates;
}else{
